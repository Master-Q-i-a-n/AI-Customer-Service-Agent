...
这是一个用于学习和面试展示的智能工单 Agent 项目，不是生产级系统。项目在传统工单系统的基础上接入 AI 服务，用来展示工单分类、回复建议、知识库问答、RAG、工具调用、历史案例记忆和 Agent 评测等能力。
...
- `LANGSMITH_API_KEY`：用于 LangSmith 评测和链路观测

你可以选择配置到系统环境变量中，也可以在项目根目录新建 `.env` 文件。Python AI 服务会通过 `python-dotenv` 自动读取 `.env`。
...
- AI 服务：FastAPI、LangChain、DashScope、Milvus Lite
- Agent 能力：工具调用、RAG、HyDE、Rerank、历史案例记忆

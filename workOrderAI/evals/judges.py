...
def _invoke_json(prompt_text: str, values: dict) -> dict:
    """渲染 judge 提示词、调用模型并解析固定 JSON 结果。"""
    # 所有 judge 都收敛到固定 JSON，后续评分和报告才能稳定解析。
...
def judge_reply(
    ...
    """用 LLM 评估回复建议的完成度、依据、共情和可执行性。"""
    # 把工具输出一并交给 judge，避免真实查询结果被误判为“模型编造”。
    scores = _invoke_json(
        REPLY_JUDGE_PROMPT,
        {
...
def judge_knowledge(question: str, answer: str, sources: list[dict]) -> dict:
    """用 LLM 评估知识问答答案质量与引用一致性。"""
    # 规则分负责命中检查，judge 负责补足正确性、完整性和引用一致性的语义判断。
    scores = _invoke_json(
...
def judge_refusal(question: str, answer: str) -> dict:
    """用 LLM 判断答案是否构成语义上的正确拒答。"""
    # 单独的拒答 judge 用来识别“同义拒答”，不要求答案写死某一句模板话术。
    return _invoke_json(
